...
## - the first column designates the class ID label
...
##    and specify the exponent which realizes the property set in that class
## 
## It outputs various conditional entropy measures based on pairwise comparison of forms.
## Same calculation method as in Ackerman and Malouf (2013) and this script has been
## checked against their data; produces same output. Also checked against Finkel and Stump's
## Principal Parts Analyzer (its comparable entropy calc function)
## 
## Andrea D. Sims
## v. 7-2-14
## 
...
doCalc.fnc = function(file, weighted){
	...
	forms = forms[,c(1,4:ncol(forms))]	#Remove irregularity summing from file for formatting reasons
	forms[,1] = paste("Class",c(1:nrow(forms)), sep="") #Change class labels because they are a pain
...
#Read in output of calcProb.fnc()
	...
	if(weighted == T){
		write.table(output, file=paste0(strsplit(file_name,".txt"), "_entropy_weighted_Rcalc.txt"), append=F, quote=F, sep="\t", row.names=F, col.names=T, fileEncoding = "UTF-8")
	...
	if(weighted == F){
		write.table(output, file=paste0(strsplit(file_name,".txt"), "_entropy_nonweighted_Rcalc.txt"), append=F, quote=F, sep="\t", row.names=F, col.names=T, fileEncoding = "UTF-8")
		temp<-aggregate(data=output, FUN=mean, entropy_AgB ~ msps_A+msps_B)
		mean<- mean(round(temp$entropy_AgB, digits=9))
